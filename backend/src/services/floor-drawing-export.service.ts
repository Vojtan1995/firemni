...
function filterLabel(filters: FloorDrawingExportFilters) {
  ...
  if (filters.sealIds?.length) parts.push(`Vybrané ucpávky: ${filters.sealIds.length}`);
  if (filters.from || filters.to) parts.push(`Období: ${filters.from ?? '—'} – ${filters.to ?? '—'}`);
...
export async function exportFloorDrawingPdf(
  ...
  const x0 = doc.x;
  const y0 = doc.y;
  doc.image(thumb, x0, y0, { width: imgW, height: imgH });
  ...
  for (const m of filtered) {
    ...
    doc.fillColor('#FFFFFF')
      .fontSize(6)
      .text(m.seal.sealNumber, cx - r, cy - 3, { width: r * 2, align: 'center' });
  ...
  doc.fontSize(9);
  doc.fillColor(STATUS_COLORS[SealStatus.draft]).text('● Rozpracováno', { continued: false });
  doc.fillColor(STATUS_COLORS[SealStatus.checked]).text('● Zkontrolováno', { continued: false });
  doc.fillColor(STATUS_COLORS[SealStatus.invoiced]).text('● Fakturováno', { continued: false });
