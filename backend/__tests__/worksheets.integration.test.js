  ...
  describe("worksheet items", () => {
    ...
    it("management can add items to a draft worksheet centrally", async () => {
      ...
      const ws = await request(app)
        .post("/api/worksheets")
        .set("Authorization", `Bearer ${vedeniToken}`)
        .send({ jobId: jobIdLocal, workerIds: [worker.id] });
      expect(ws.status).toBe(201);

      const seal = await createSeal("6");
      const add = await request(app)
        .post(`/api/worksheets/${ws.body.id}/items`)
        .set("Authorization", `Bearer ${vedeniToken}`)
        .send({ sealEntryIds: [seal.entries[0].id] });
      expect(add.status).toBe(201);
      expect(add.body).toHaveLength(1);

      const detail = await request(app)
        .get(`/api/worksheets/${ws.body.id}`)
        .set("Authorization", `Bearer ${vedeniToken}`);
      expect(detail.status).toBe(200);
    ...
    it("populate status filter adds only matching draft items", async () => {
      const ws = await request(app)
        .post("/api/worksheets")
        .set("Authorization", `Bearer ${workerToken}`)
        .send({ jobId: jobIdLocal });
      ...
      const draftSeal = await createSeal("7", populateFloorId);
      const checkedSeal = await createSeal("8", populateFloorId);
      await markSealChecked(app, vedeniToken, workerToken, checkedSeal.id);

      const populate = await request(app)
        .post(`/api/worksheets/${ws.body.id}/populate`)
        .set("Authorization", `Bearer ${workerToken}`)
        .send({ floorIds: [populateFloorId], status: "draft" });
      expect(populate.status).toBe(201);

      const addedEntryIds = populate.body.items.map((item) => item.sealEntryId);
      expect(addedEntryIds).toContain(draftSeal.entries[0].id);
      expect(addedEntryIds).not.toContain(checkedSeal.entries[0].id);
    });
